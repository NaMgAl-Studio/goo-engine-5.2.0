...
/* Goo Engine "Shader Info" bridge: node-tree code cannot acquire EEVEE-Next's forward light-eval
 * resources, so the forward pipeline (surf_forward) computes Goo's separable lighting components
 * with the bound light resources and stores them here; the Shader Info node reads them. `valid`
 * is left false (zero-initialized) in pipelines that do not fill it, so the node stays neutral. */
/* Per-visible-light records populated by the surface shader (which has the light resources),
 * so the Shader Info node can reproduce Goo's exact per-light light-group test:
 *   contribute = (group_bits & node_mask) != 0
 * matching Goo's `calc_shader_info` exactly: a light with all-zero group bits contributes to
 * nothing (this also naturally excludes EEVEE-Next's world-sun placeholder lights, which do not
 * exist in Goo). A multi-group light contributes if ANY of its groups is selected (no double
 * counting). Diffuse/half-lambert and cast/self shadows are filtered independently by the node's
 * two masks. Records store the UNSHADOWED lighting (Goo's Diffuse Shading has no shadow factor)
 * plus the scalar shadow visibility, so shadows only affect the dedicated shadow outputs.
 * Visible lights beyond GOO_MAX_LIGHTS spill into an always-on overflow accumulator. */
#define GOO_MAX_LIGHTS 512
struct GooShaderInfoData {
...
GooShaderInfoData g_goo_shader_info;

/* Goo Engine "Set Depth" bridge: the Set Depth node writes the requested view-space depth here
 * (and sets `g_set_depth_written`); a dedicated non-early-Z surface variant converts it to a
 * screen depth and writes gl_FragDepth. Zero-initialized `written` keeps normal materials
 * unaffected. */
float g_set_depth;
bool g_set_depth_written;
