  ...
  for (bNode &node : ntree->nodes) {
    if (node.runtime->tmp_flag >= 0) {
      ...
      for (const auto [socket_index, sock] : node.inputs.enumerate()) {
        /* Set Depth is a single-closure passthrough; its second input (View Depth, a float) is not
         * part of the closure weight tree and must be skipped here. */
        if (node.type_legacy == SH_NODE_SET_DEPTH && socket_index != 0) {
          continue;
        }
        bNodeSocket *tosock;
        bNode *tonode;
