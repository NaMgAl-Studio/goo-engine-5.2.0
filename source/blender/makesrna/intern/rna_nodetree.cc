...
static void def_sh_shader_info(BlenderRNA *brna, StructRNA *srna)
{
  ...
  RNA_def_property_ui_text(prop, "Use Own Light Groups", "Use per-node light groups");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_update");
}

static void def_sh_sdf_primitive(BlenderRNA *brna, StructRNA *srna)
{
  static const EnumPropertyItem sdf_primitive_items[] = {
      ...
      {SHD_SDF_3D_HEX_PRISM_INCIRCLE, "HEX_PRISM_INCIRCLE_3D", 0, "Hex Prism Incircle", "3d sdf hex prism incircle"},
      ...
      {SHD_SDF_2D_POINT_TRIANGLE, "POINT_TRIANGLE_2D", 0, "Point Triangle", "2d sdf 3 point triangle"},
      {SHD_SDF_2D_QUAD, "QUAD_2D", 0, "Quad", "2d sdf quad"},
      {SHD_SDF_2D_PARABOLA, "PARABOLA_2D", 0, "Parabola", "2d sdf parabola"},
      {SHD_SDF_2D_PARABOLA_SEGMENT, "PARABOLA_SEGMENT_2D", 0, "Parabola Segment", "2d sdf parabola segment"},
  ...
  RNA_def_property_enum_items(prop, sdf_primitive_items);
...
{
  static const EnumPropertyItem sdf_op_items[] = {
      ...
      {SHD_SDF_OP_PULSE, "PULSE", 0, "Hermite Pulse", "Hermite pulse between min and max SDF values"},
  ...
  RNA_def_property_enum_items(prop, sdf_op_items);
...
{
  static const EnumPropertyItem sdf_vector_op_items[] = {
      {0, "", 0, N_("Vector Grid Ops"), ""},
      {SHD_SDF_VEC_OP_REFLECT, "VEC_REFLECT", 0, "Plane Reflect", "Reflect position with a plane and offset"},
      {SHD_SDF_VEC_OP_MIRROR, "VEC_MIRROR", 0, "Mirror", "Mirror at axis-aligned planes at a distance from the origin"},
      {SHD_SDF_VEC_OP_POLAR, "VEC_POLAR", 0, "Polar", "Repeat around the origin by a fixed angle"},
      {SHD_SDF_VEC_OP_REPEAT_INF, "VEC_REPEAT_INF", 0, "Repeat Infinite", "Repeat position infinitely with spacing control"},
      {SHD_SDF_VEC_OP_REPEAT_INF_MIRROR, "VEC_REPEAT_INF_MIRROR", 0, "Repeat Infinite Mirror", "Repeat infinitely, mirroring every second cell so boundaries match"},
      {SHD_SDF_VEC_OP_REPEAT_FINITE, "VEC_REPEAT_FINITE", 0, "Repeat Finite", "Repeat position with limits"},
      ...
      {SHD_SDF_VEC_OP_EXTRUDE, "VEC_EXTRUDE", 0, "Extrude", "Extrude position"},
      {SHD_SDF_VEC_OP_TWIST, "VEC_TWIST", 0, "Twist", "Twist around XY"},
      {SHD_SDF_VEC_OP_SWIRL, "VEC_SWIRL", 0, "Swirl", "Swirl around XY"},
      {SHD_SDF_VEC_OP_PINCH_INFLATE, "VEC_PINCH_INFLATE", 0, "Pinch Inflate", "Pinch inflate around XYZ"},
      {SHD_SDF_VEC_OP_RADIAL_SHEAR, "VEC_RADIAL_SHEAR", 0, "Radial Shear", "Radially shear around XY"},
      {SHD_SDF_VEC_OP_BEND, "VEC_BEND", 0, "Bend", "Bend position"},
      {0, "", 0, N_("UV Ops"), ""},
      {SHD_SDF_VEC_OP_ROTATE_UV, "UV_ROTATE", 0, "UV Rotate", "Rotate UV around center point [.5,.5]"},
      {SHD_SDF_VEC_OP_SCALE_UV, "UV_SCALE", 0, "UV Scale", "Scale UV from center point [.5,.5]"},
      {SHD_SDF_VEC_OP_GRID, "UV_GRID", 0, "UV Grid", "UV grid"},
      {SHD_SDF_VEC_OP_RND_UV, "UV_RAND_ROTATE", 0, "UV Random Rotate", "Rotate UV coordinates by 90 degrees in grid"},
      {SHD_SDF_VEC_OP_RND_UV_FLIP, "UV_RAND_FLIP", 0, "UV Random Flip", "Flip and rotate UV coordinates"},
  ...
  prop = RNA_def_property(srna, "operation", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "operation");
  RNA_def_property_enum_items(prop, sdf_vector_op_items);
  RNA_def_property_ui_text(prop, "Operation", "");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_ShaderNode_socket_update");
...
{
  static const EnumPropertyItem prop_hexagon_coords[] = {
      {SHD_HEXAGON_COORDS_XY, "XY", 0, "XY Position", "Cell ID and UV output XY values"},
      {SHD_HEXAGON_COORDS_HEX, "HEX", 0, "Hex Position", "Cell ID and UV output HEX values"},
      {0, nullptr, 0, nullptr, nullptr},
  };
  static const EnumPropertyItem prop_hexagon_value[] = {
      {SHD_HEXAGON_VALUE_HEX, "HEX", 0, "Hexagons", "Value based on hexagon distance function"},
      {SHD_HEXAGON_VALUE_SDF, "SDF", 0, "SDF Hexagons", "Value based on SDF hexagon distance"},
      {SHD_HEXAGON_VALUE_DOT, "DOT", 0, "Dots", "Value based on coordinate length"},
      {0, nullptr, 0, nullptr, nullptr},
  };
  static const EnumPropertyItem prop_hexagon_direction[] = {
      {0, "HORIZONTAL", 0, "Horizontal", "Hexagons point horizontally"},
      {1, "VERTICAL", 0, "Vertical", "Hexagons point vertically"},
      {2, "HORIZONTAL_TILED", 0, "Horizontal Tiled", "Fit hexagon into a square aspect"},
      {3, "VERTICAL_TILED", 0, "Vertical Tiled", "Fit hexagon into a square aspect"},
      {0, nullptr, 0, nullptr, nullptr},
  };
  ...
  RNA_def_property_enum_items(prop, prop_hexagon_coords);
  ...
  RNA_def_property_enum_items(prop, prop_hexagon_value);
  ...
  RNA_def_property_enum_items(prop, prop_hexagon_direction);
  RNA_def_property_ui_text(prop, "Direction", "Direction of the hexagon pattern");
  RNA_def_property_update(prop, 0, "rna_Node_update");

  prop = RNA_def_property(srna, "use_clamp", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "use_clamp", 1);
  RNA_def_property_ui_text(prop, "Clamp", "Clamp the value output to the 0..1 range");
  RNA_def_property_update(prop, 0, "rna_Node_update");
}
...
static void def_sh_light_info(BlenderRNA *brna, StructRNA *srna)
{
  ...
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Light", "Light object to read color and power from");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_update");
}

static void def_sh_water_ripples(BlenderRNA *brna, StructRNA *srna)
{
  static const EnumPropertyItem prop_water_ripples_mode[] = {
  ...
  prop = RNA_def_property(srna, "mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "mode");
  RNA_def_property_enum_items(prop, prop_water_ripples_mode);
  RNA_def_property_ui_text(prop, "Mode", "Water effect mode");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_ShaderNode_socket_update");
}
