...
static void rna_def_light(BlenderRNA *brna)
{
  ...
  /* Goo Engine: per-light light-group membership for the Shader Info node. */
...
static void rna_def_light_shadow(StructRNA *srna, bool sun)
{
  ...
  RNA_def_property_ui_text(
      prop, "Contact Shadow Distance", "World space distance in which to search for occluders");
  RNA_def_property_update(prop, 0, "rna_Light_update");
