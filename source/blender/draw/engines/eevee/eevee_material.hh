...
/* A material flagged MA_LEGACY_OPAQUE (legacy EEVEE "OPAQUE", set by versioning) must ignore its
 * transparency closure and render fully opaque, matching legacy/Goo. Materials created in
 * EEVEE-Next are never flagged, so their dithered/blended alpha is unaffected. */
static inline bool gpu_material_is_transparent(const GPUMaterial *gpumat)
{
  if (!GPU_material_flag_get(gpumat, GPU_MATFLAG_TRANSPARENT)) {
    return false;
  }
  const auto *ma = GPU_material_get_material(const_cast<GPUMaterial *>(gpumat));
  return ma == nullptr || (ma->flag & MA_LEGACY_OPAQUE) == 0;
}
...
struct Material {
  ...
  bool has_surface;
  bool has_volume;
  /** Surface reflections ignore shadow-map winners from the same full resource ID. */
  bool use_shadow_id = false;
  MaterialPass shadow;
