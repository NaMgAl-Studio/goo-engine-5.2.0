                  ...
                  [[front_facing]] const bool front_face)
{
  ...
  /* Goo Shader Info: compute the separable lighting components with the bound light resources
   * before the node tree runs, so the Shader Info node can read them via g_goo_shader_info.
   * Compiled only for materials that actually contain the node (MAT_GOO_SHADER_INFO). */
#ifdef MAT_GOO_SHADER_INFO
  eevee::goo_shader_info_compute(view, resource_id, gl_FragCoord.xy);
#endif
...
/* Goo Set Depth variant of surf_forward: identical shading, but WITHOUT early-Z so it can write a
 * per-pixel gl_FragDepth from the Set Depth node's requested view-space depth (bridged through
 * g_set_depth). Selected for materials flagged GPU_MATFLAG_SET_DEPTH. */
[[fragment]]
void surf_forward_set_depth([[resource_table]] PipelineConstants & /*pipe*/,
                            [[resource_table]] SurfaceForward & /*srt*/,
                            [[resource_table]] LightEvalIterator & /*lights*/,
                            [[resource_table]] LightprobeRenderData & /*lightprobes*/,
                            [[resource_table]] LightprobePlaneRenderData & /*lightprobe_planes*/,
                            ...
                            [[frag_coord]] const float4 frag_co,
                            [[out]] SurfaceForwardFragOut &frag_out,
                            [[frag_depth(any)]] float &out_frag_depth,
                            [[front_facing]] const bool front_face)
{
  auto &interp_flat = interface_get(eevee_geom_iface_info, interp_flat);
  draw::ID id{interp_flat.resource_id_raw};
  ...
  /* Goo Set Depth: write the node-provided view-space depth as a screen depth (no-op otherwise). */
  out_frag_depth = g_set_depth_written ? (1.0f - view.depth_view_to_screen(-g_set_depth)) :
                                         gl_FragCoord.z;
