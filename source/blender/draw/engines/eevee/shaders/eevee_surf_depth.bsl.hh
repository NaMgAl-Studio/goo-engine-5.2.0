...
/* Goo Set Depth prepass variant: writes the same per-pixel offset depth (gl_FragDepth) as the
 * Set Depth shading variant so the shading pass DEPTH_EQUAL test still matches. Non-templated
 * (no velocity variant); selected for materials flagged GPU_MATFLAG_SET_DEPTH. */
[[fragment]]
void surf_depth_set_depth([[resource_table]] PipelineConstants &pipe,
                          [[resource_table]] SurfaceDepth & /*srt*/,
                          [[resource_table]] const Uniform &uni,
                          [[resource_table]] const Sampling &sampling,
                          [[resource_table]] const UtilityTexture & /*util_tx*/,
                          [[resource_table]] const draw::View &views,
                          [[frag_coord]] const float4 /*frag_co*/,
                          [[out]] SurfaceDepthFragOut<false> &frag_out,
                          ...
                          [[front_facing]] const bool front_face)
{
  ...
  if (pipe.use_transparency) [[static_branch]] {
    ...
    float threshold = hashed_transparency::alpha_threshold(
        uni.pipeline_buf.alpha_hash_scale, noise_offset, g_data.P);
    if (average(g_transmittance) > threshold) {
      gpu_discard_fragment();
      return;
    }
  }

  /* Reverse-Z: the depth buffer stores 1 - screen depth (the rasterized fallback gl_FragCoord.z is
   * already reversed), so the Set Depth offset must be reversed too. Prepass + shading pass write
   * the identical value, keeping DEPTH_EQUAL consistent. */
