                 ...
                 float threshold)
{
  ...
  this->contact_thickness = la->contact_thickness;
...
void LightModule::add_world_sun_light(const ObjectKey &key, bool use_diffuse, bool use_glossy)
{
  ...
  la.energy = 1.0f;
  /* Goo Engine: world-sun placeholders do not exist in Goo; zero group bits exclude them from the
   * Shader Info per-light loop (the DNA default is the builtin default group otherwise). */
  la.light_group_bits[0] = la.light_group_bits[1] = la.light_group_bits[2] =
      la.light_group_bits[3] = 0;
