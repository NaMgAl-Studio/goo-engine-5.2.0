    ...
    default:
      switch (pipeline_type) {
        ...
        case MAT_PIPE_PREPASS_DEFERRED:
          ...
          /* Goo Set Depth: the prepass must write the same offset depth as the shading
           * pass (so DEPTH_EQUAL still matches). Use the variant's own create-info so its
           * gl_FragDepth output/depth-write is declared. */
          if (use_set_depth && (pipeline_type == MAT_PIPE_PREPASS_FORWARD ||
                                pipeline_type == MAT_PIPE_PREPASS_DEFERRED))
          {
            pipeline_info_name = "eevee_surf_depth_set_depth_infos_";
            info.fragment_function("eevee_surf_depth_set_depth");
          }
          break;
        ...
        case MAT_PIPE_FORWARD:
          pipeline_info_name = use_set_depth ? "eevee_surf_forward_set_depth_infos_" :
          ...
          info.fragment_function(use_set_depth ? "eevee_surf_forward_set_depth" :
                                                 "eevee_surf_forward");
          break;
        default:
...
void ShaderModule::material_create_info_amend(GPUMaterial *gpumat, GPUCodegenOutput *codegen_)
{
  ...
  if (gpu_material_is_transparent(gpumat)) {
  ...
  /* Legacy OPAQUE is a surface shader variant. It keeps the external surface opaque while
  ...
  if (GPU_material_flag_get(gpumat, GPU_MATFLAG_GOO_SHADER_INFO)) {
    info.define("MAT_GOO_SHADER_INFO");
  }
  ...
  SlotAllocator slots = add_pipeline_create_info(
      info, pipeline_type, geometry_type, use_shader_to_rgba, use_set_depth);
  ...
  if (GPU_material_flag_get(gpumat, GPU_MATFLAG_SET_DEPTH)) {
    /* Goo Set Depth: write gl_FragDepth from the node's requested view-space depth. */
    info.define("MAT_SET_DEPTH");
    /* Enable depth write on the passes that write gl_FragDepth (deferred/hybrid gbuffer + the
     * matching prepass, so DEPTH_EQUAL still holds). */
    if (pipeline_type == MAT_PIPE_DEFERRED || pipeline_type == MAT_PIPE_PREPASS_DEFERRED ||
        pipeline_type == MAT_PIPE_PREPASS_FORWARD)
    {
