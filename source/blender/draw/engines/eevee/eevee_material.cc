                                               ...
                                               eMaterialProbe probe_capture)
{
  ...
  const bool is_transparent = gpu_material_is_transparent(matpass.gpumat);
                                        ...
                                        bool has_motion)
{
  ...
  Material &mat = material_map_.lookup_or_add_cb(material_key, [&]() {
    ...
    /* Goo/legacy EEVEE: materials authored with "Shadow Mode: None" (blend_shadow) must not cast
     * shadows (e.g. sky-dome meshes enclosing the scene). EEVEE-Next removed this option, so
     * honor the legacy DNA value; new materials always have MA_BS_SOLID. */
    const bool legacy_no_shadow = blender_mat->blend_shadow == MA_BS_NONE;
    if (!(ob->visibility_flag & OB_HIDE_SHADOW) && !legacy_no_shadow) {
